...
const ManagerEditor: React.FC<ManagerEditorProps> = ({ checklists, onUpdateChecklists }) => {
    ...
    return (
        <div className="p-6 bg-[var(--color-bg-primary)] rounded-xl shadow-2xl space-y-6">
            <h2 className="text-2xl font-extrabold text-[var(--color-text-accent)] border-b border-[var(--color-border-primary)] pb-2">📋 Checklist Manager</h2>

            <div className="flex flex-col space-y-3 md:flex-row md:space-y-0 md:space-x-4">
                <div className="flex-1">
                    <label className="block text-sm font-medium text-[var(--color-text-secondary)]">Select List to Edit:</label>
                    <select onChange={(e) => setSelectedListId(e.target.value)} value={selectedListId} className="mt-1 block w-full pl-3 pr-10 py-2 text-base bg-[var(--color-bg-secondary)] border-[var(--color-border-secondary)] focus:outline-none focus:ring-[var(--color-border-accent)] focus:border-[var(--color-border-accent)] sm:text-sm rounded-md shadow-sm">
                        {checklists.map(list => <option key={list.id} value={list.id}>{list.name}</option>)}
                    </select>
                </div>
                <div className="flex-1">
                    <label className="block text-sm font-medium text-[var(--color-text-secondary)]">Create New List:</label>
                    <div className="flex mt-1">
                        <input type="text" placeholder="e.g., Daily Prep" value={newListName} onChange={(e) => setNewListName(e.target.value)} className="flex-1 min-w-0 block w-full px-3 py-2 bg-[var(--color-bg-secondary)] border border-[var(--color-border-secondary)] rounded-l-md focus:ring-[var(--color-border-accent)] focus:border-[var(--color-border-accent)]"/>
                        <button onClick={handleCreateList} className="px-4 py-2 bg-[var(--color-bg-accent-primary)] text-[var(--color-text-inverse)] text-sm font-medium rounded-r-md hover:bg-[var(--color-bg-accent-primary-hover)] transition">Create</button>
                    </div>
                </div>
            </div>
            ...
            {selectedList && (
                <div className="flex justify-end pt-2">
                    <button onClick={handleDeleteList} disabled={['health', 'boh_supervisor_audit'].includes(selectedList.id)} className={`px-4 py-2 text-sm font-semibold rounded-lg shadow-md transition ${['health', 'boh_supervisor_audit'].includes(selectedList.id) ? 'bg-[var(--color-error-bg)] text-[var(--color-error-text)] cursor-not-allowed' : 'bg-[#DC2626] text-white hover:bg-[#B91C1C]'}`} title={['health', 'boh_supervisor_audit'].includes(selectedList.id) ? "Cannot delete core compliance lists." : "Permanently delete this checklist."}>
